...
import { Magnet, Sparkles, Plus, Trash2 } from 'lucide-react';
...
const MarketingROI: React.FC = () => {
  ...
  const { messages, isTyping, sendMessage } = useAIChat(
    "👋 I'm your AI assistant for lead magnet creation! I can help you with lead magnet creator tasks using natural language. Just tell me what you need!"
  );
  ...
  const quickActions = [
    { label: '✨ Create new', prompt: 'Create a new lead magnet creation item' },
    ...
    >
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold flex items-center gap-2" style={{ color: '#3b82f6' }}>
        ...
        {items.length === 0 ? (
          <div className="text-center py-16 text-gray-500">
            <Magnet className="w-12 h-12 mx-auto mb-4 opacity-30" />
            <p className="text-lg mb-2">No items yet</p>
            <p className="text-sm">Use AI to create lead magnet creation items!</p>
            <p className="text-xs mt-3 text-gray-600">
              Try: "Create a new lead for my project"
            </p>
          </div>
        ) : (
